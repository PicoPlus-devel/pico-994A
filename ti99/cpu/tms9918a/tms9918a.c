...
u8 MaxSprites[2] DTCM_DATA = {4, 32};     // Normally the TMS9918a only shows 4 sprites on a line... for emulation we bump this up if configured


// One 256x192 8bpp frame. Upstream keeps a second buffer for the DS's frame-blend
// option; this port converts the finished frame to the display in one pass at
// vblank instead (see TI99UpdateScreen in main.cpp), so the 48K second copy - and
// the tearing the blend was papering over - are both gone.
// -------------------------------------------------------------------------------------
// Upstream keeps two full 256x192 frames here (96K) and blits one at vblank. On RP2350
// that is a large slice of a 512K SRAM already carrying a 150K framebuffer, so this
// port renders one scanline at a time: RefreshLineN fills XBuf, TI99RenderLine converts
// it straight into the display row, and the next line reuses the same 256 bytes.
//
// This is safe against tearing because the emulation loop paces to vsync *before*
// running a frame, so the writer starts at line 0 during vblank and stays ahead of the
// scanout DMA - the same arrangement pico-smsplus uses for its 256x192 output.
// -------------------------------------------------------------------------------------
static u8 XBuf_Line[256] ALIGN(32);
u8 *XBuf_A = XBuf_Line;
u8 *XBuf DTCM_DATA = XBuf_Line;

u32 lutTablehh[256][16];    // Look up table for colors - pre-generated for maximum speed!
u32 fastBackgroundLut[256] DTCM_DATA; // For when the color is background - happens often enough

u8 OH DTCM_DATA = 0;
u8 IH DTCM_DATA = 0;

u8 scan_collisions_every DTCM_DATA = 32;
...
// ---------------------------------------------------------------------------------------
tScrMode SCR[MAXSCREEN+1] DTCM_DATA  = {
...
void (*RefreshLine)(u8 uY) DTCM_DATA = RefreshLine0;

/** Palette9918[] ********************************************/
/** 16 standard colors used by TMS9918/TMS9928 VDP chips.   **/
/*************************************************************/
// TI palette index currently selected as the backdrop (VDP register 7, low nibble).
// Pixels of colour 0 in XBuf take this colour when the frame is converted.
u8 ti99_backdrop_color DTCM_DATA = 0;

u8 TMS9918A_palette[16*3] DTCM_DATA  = {
...
u8 *pVDPVidMem;                                         // VDP video memory... 16K, heap allocated

u16 CurLine     DTCM_DATA;      // Current scanline
u8 VDP[16]      DTCM_DATA;      // VDP Registers
u8 VDPStatus    DTCM_DATA;      // VDP Status
u8 VDPDlatch    DTCM_DATA;      // VDP register D Latch
u16 VAddr       DTCM_DATA;      // VDP Video Address
u8 VDPCtrlLatch DTCM_DATA;      // VDP control latch key
u8 *ChrGen      DTCM_DATA;      // VDP tables (screens)
u8 *ChrTab      DTCM_DATA;      // VDP tables (screens)
u8 *ColTab      DTCM_DATA;      // VDP tables (screens)
u8 *SprGen      DTCM_DATA;      // VDP tables (sprites)
u8 *SprTab      DTCM_DATA;      // VDP tables (sprites)
u8 ScrMode      DTCM_DATA;      // Current screen mode
u8 FGColor      DTCM_DATA;      // Foreground Color
u8 BGColor      DTCM_DATA;      // Background Color

// Sprite and Character Masks for the VDP
u16 ChrTabM     DTCM_DATA = 0x3FFF;
u16 ColTabM     DTCM_DATA = 0x3FFF;
u16 ChrGenM     DTCM_DATA = 0x3FFF;
u16 SprTabM     DTCM_DATA = 0x3FFF;
 ...
 ********************************************************************************/
u8 VDP_RegisterMasks[] DTCM_DATA = { 0x03, 0xfb, 0x0f, 0xff, 0x07, 0x7f, 0x07, 0xff };
byte SprHeights[4] DTCM_DATA = { 8,16,16,32 };
...
ITCM_CODE byte Write9918(u8 iReg, u8 value)
{
  ...
  /* The TI99 is normally 16K but it's possible to use the VDP chip in 4K mode */
  VRAMMask = (iReg==1) && ((VDP[1]^value) & TMS9918_REG1_RAM16K) ? 0 : TMS9918_VRAMMask;  // Setting zero here forces re-computation of the VDP tables below
  ...
  switch (iReg) {
    ...
    case  7:
      FGColor=value>>4;
      BGColor=value&0x0F;
      // Colour 0 is "transparent" and shows the backdrop. Upstream writes the DS's
      // hardware backdrop register here; this port records the palette index and the
      // frame conversion in main.cpp substitutes it wherever XBuf holds a 0.
      ti99_backdrop_color = BGColor;
      break;
  }
...
/*************************************************************/
u8 frameSkipIdx DTCM_DATA = 0;
// Frameskip masks: a frame renders when (frameSkipIdx & mask) != 0. Index 0 is "off",
// but 0xFF still drops one frame in every 256 - a visible hiccup every four seconds.
// RP2350 has the headroom to render every frame, so index 0 is handled as a special
// case at the two test sites below and this entry is only a placeholder.
u8 frameSkip[3] DTCM_DATA = {0xFF, 0x03, 0x01};   // Frameskip OFF, Light, Aggressive
#define FRAME_IS_RENDERED  (myConfig.frameSkip == 0 || (frameSkipIdx & frameSkip[myConfig.frameSkip]) != 0)

u16 tms_num_lines  DTCM_DATA = TMS9918_LINES;
u16 tms_start_line DTCM_DATA = TMS9918_START_LINE;
u16 tms_end_line   DTCM_DATA = TMS9918_END_LINE;
u16 tms_cpu_line   DTCM_DATA = TMS9918_LINE;
...
ITCM_CODE byte Loop9918(void)
{
  ...
  if ((CurLine >= tms_start_line) && (CurLine < tms_end_line))
  {
      ...
      else
      {
          ...
          TI99RenderLine((u8)(CurLine - tms_start_line));
      }
...
void Reset9918(void)
{
    ...
    {
        scan_collisions_every = 32;  // How often to scan for sprite collisions (RP2350 can afford the accurate rate)
    }
