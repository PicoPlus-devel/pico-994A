...
#include "sn76496_local.h"

#define MAX_OUTPUT  0x7FFF
#define STEP        0x10000

// Noise LFSR taps. The defaults below are pico-smsplus's, tuned for the PSG
// integrated into the Sega VDP. The TI-99/4A has a standalone SN94624/TMS9919,
// which uses a shorter shift register and so a different noise timbre - DS994a
// asks FluBBa's core for chip type 1 (SN76496) rather than 0 (SMS VDP) for
// exactly this reason. Overridable so the TI values can be dropped in once the
// difference has actually been listened to on hardware; leaving the proven
// values as the default avoids trading working noise for guessed-at taps.
#ifndef FB_WNOISE
