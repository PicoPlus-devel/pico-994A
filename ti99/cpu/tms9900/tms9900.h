...
extern u8 *MemSuperCartRAM;   // 32K of Super Cart banked RAM, NULL unless that cart type is active
...
// ---------------------------------------------------------------------------------------------------------
// Replaces upstream's 128KB CompareZeroLookup16[] table (see tms9900.c). The
// table held exactly this expression for every 16-bit index; computing it is both
// smaller and, on Cortex-M33, cheaper than the load it replaces.
static inline u16 COMPARE_ZERO_16(u16 v)
{
    if (v == 0)      return ST_EQ;
    if (v < 0x8000)  return ST_LGT | ST_AGT;   // positive, non-zero
    return ST_LGT;                             // negative: logical > but not arithmetic >
}
...
enum _MEM_TYPE
{
    ...
    MF_PCODE,       // This is the P-CODE emulation handling
