...
u8 *MemSAMS             DTCM_DATA = 0;  // Allocated to support 512K for DS-Lite and 1MB/2MB for DSi and above
SAMS theSAMS            DTCM_DATA;      // The entire state of the SAMS memory map handler
u16 sams_highwater_bank DTCM_DATA = 0;  // To track how far into SAMS memory we have used
...
void SAMS_Initialize(void)
{
    ...
    // ------------------------------------------------------------------------
    // SAMS needs a megabyte or more of backing store, which only exists on this
    // hardware when a PSRAM chip is fitted. Boards without it stay at the plain
    // 32K expansion (which is part of the base memory map and always present).
    // The allocation is handed out by the front end so it lands in PSRAM rather
    // than the SRAM heap the menu and framebuffer are competing for.
    // ------------------------------------------------------------------------
    {
        u32 wantBanks = 0;
        switch (myConfig.machineType)
        {
            case MACH_TYPE_SAMS_1MB: wantBanks = 256;  break;
            case MACH_TYPE_SAMS_2MB: wantBanks = 512;  break;
            case MACH_TYPE_SAMS_4MB: wantBanks = 1024; break;
            case MACH_TYPE_SAMS_8MB: wantBanks = 2048; break;
            default:                 wantBanks = 0;    break;   // MACH_TYPE_NORMAL32K
        }

        if (MemSAMS) { ti99_psram_free(MemSAMS); MemSAMS = NULL; }

        if (wantBanks)
        {
            MemSAMS = (u8 *)ti99_psram_alloc(wantBanks * 0x1000);
            if (MemSAMS == NULL)
            {
                // No PSRAM (or not enough) - fall back rather than run with a
                // null bank pointer the CPU would happily dereference.
                myConfig.machineType = MACH_TYPE_NORMAL32K;
                wantBanks = 0;
            }
        }
        theSAMS.numBanks = wantBanks;
    }

    if (theSAMS.numBanks == 0)
    {
        // Nothing more to set up - the plain 32K expansion is already mapped.
        return;
    }
...
// ---------------------------------------------------------------------
// Release the PSRAM backing store. Called when the emulator tears down between games -
// without it a SAMS machine leaks its megabytes on every game switch.
void SAMS_Release(void)
{
    if (MemSAMS) { ti99_psram_free(MemSAMS); MemSAMS = NULL; }
    theSAMS.numBanks = 0;
}
