...
lowzip_state st         DTCM_DATA;
yxml_t xml              DTCM_DATA;
char xml_value[64]      DTCM_DATA;
Layout_t cart_layout    DTCM_DATA;
...
u8 rpk_load(const char* filename)
{
    ...
    if (errors)
    {
        ...
        // MemCART is sized to the cart here, not a fixed 512K as upstream, so this
        // clear has to respect the actual allocation.
